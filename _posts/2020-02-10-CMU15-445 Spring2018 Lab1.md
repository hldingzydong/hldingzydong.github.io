...
### Specification
BPM与disk打交道,它从disk中获取所需的page并将其放置于memory中,当memory中某一page变脏之后,还要将其写回disk中。BPM标明哪些memory中的page被pin住,所谓pin住是指像被钉子钉在memory中,不能将其作为Victim,除非不断Unpin它直到count=0(没有人access该page).  
