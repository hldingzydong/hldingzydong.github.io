...
#### Insert
###### FindLeafPage
给出key,找到该key所在的叶结点，在这个函数中体现了B+Tree的RLatch的策略:
```c++
...
这里还需要实现**Split**函数来实现将某个node split,还需要实现**InsertIntoParent**将new node插入到parent中。
```c++
...
- root node也满了,需要insert parnet,但是root没有parent,那需要新建一个root,之前的root变成了非root的结点(此时需要更新root_page_id).我们还知道递归一定要有一个递归终止点,那这种case往往是最终的终止点。不可能从root再往上递归了。
...
#### Remove
我们在删除某一KV时,也要更新size,此时需要判断该node的size是否小于minSize.当小于minSize时,有两种策略:
...
- 当前node是root,则AdjustRoot(node)
- 判断是否小于minSize
- 拿到父结点
- 根据父结点找到所需的兄弟结点,根据当前结点在父结点中的位置找兄弟结点,若当前结点在父结点的头部,就找它后面的兄弟结点;若不在头部,就找前面的兄弟结点。
- 找到兄弟结点后,判断应该Coalesce还是Redistribute
```c++
bool BPlusTree<KeyType, ValueType, KeyComparator>::
